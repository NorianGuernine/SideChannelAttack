#define NB_WORD_DATA 16
#define DATA 100
#define NB_ROUND 10

// SBOX 8bits
const unsigned char SBOX[] = {  
                            };

// Constant Matrix RCON
const unsigned char RCON[NB_ROUND][4] = {   {0x01, 0x00, 0x00, 0x00}, {0x02, 0x00, 0x00, 0x00}, {0x04, 0x00, 0x00, 0x00},
                                            {0x08, 0x00, 0x00, 0x00}, {0x10, 0x00, 0x00, 0x00}, {0x20, 0x00, 0x00, 0x00},
                                            {0x40, 0x00, 0x00, 0x00}, {0x80, 0x00, 0x00, 0x00}, {0x1B, 0x00, 0x00, 0x00},
                                            {0x36, 0x00, 0x00, 0x00}
                                   };

void ShiftRows(unsigned char* myplaintext);
unsigned char GF_Mult_2(unsigned char input);
unsigned char GF_Mult_3(unsigned char input);
void MixColumns(unsigned char* myplaintext);
void KeySchedule(unsigned char* mycipherkey, unsigned char round);
void AES_Cipher(unsigned char* PlainText, unsigned char* CipherKey);
void AddRoundKey(unsigned char* PlainText, unsigned char* RoundKey);
void SubBytes(unsigned char* PlainText);
